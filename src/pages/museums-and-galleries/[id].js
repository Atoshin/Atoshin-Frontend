...
export default function ArtCenter({artCenter, relatedAssets}) {
    ...
    const showData = () => {
        // matches1 ? 2 : matches2 ? 3 : matches3 ? 3 : matches4 ? 4 : 5
        if (matches1) {
            if (artCenter.medias.filter(image => image.main !== 1 && image.galleryLargePicture !== 1).length + Object.keys(artCenter.videoLinks).length <= 2) {
                return mapData()
            } else {
                return slider()
            }
        ...
        <>
            ...
            <div className={classes.headerSection}>
                ...
                <div className={classes.socialMediaSec}>
                    ...
                    {artCenter.linkedin &&
                    <Link href={artCenter.linkedin}>
                        <a target='_blank'>
                            <div style={!artCenter.instagram && !artCenter.website ?
                                {cursor: "pointer"}
                                :
                                !artCenter.instagram ?
                                    {cursor: "pointer"}
                                    :
                                    !artCenter.website ?
                                        {cursor: "pointer", marginRight: 35.62}
                                        :
                                        {cursor: "pointer", marginRight: 35.62}}>
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
            ...
            <div className={matches1 ? classes.sliderSec1 : classes.sliderSec}>
                {
                    Object.keys(artCenter.videoLinks).length > 0 ?
                        <VideoShow/>
                        :
                        artCenter.medias ?
                            <GallerySlider data={artCenter.medias} setSelectedImg={setSelectedImg}/>
                            : ''
                }
            </div>
...
export async function getServerSideProps({query}) {
    ...
    } = await axios.get(`${process.env.NEXT_PUBLIC_APP_URL}/api/art-center/${artCenterId}`)
